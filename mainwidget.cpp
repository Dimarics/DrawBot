    ...
    laser_active(false), m_pointsPath("/"), m_font("Segoe UI", 12), m_tool(Pen)
{
    ...
    ui->sectionWidget->addTab("Добавить объект", ui->textPage);
    ...
    connect(ui->portBox->serialPort(), &QSerialPort::readyRead, this, &MainWidget::readPort);

    // панель меню
    connect(ui->open, &QAction::triggered, this, [this]()
    {
    ...
    connect(ui->stop, &QPushButton::clicked, this, &MainWidget::stop);

    ui->fontBox->setCurrentText("Segoe UI");
}

QList<QPointF> MainWidget::circle(const qreal radius, const QPointF &offset, const quint16 resolution) const
{
...
//-------------------------------------------------
void MainWidget::setTool(Tool tool)
{
    m_tool = tool;
}

void MainWidget::readPort()
{
